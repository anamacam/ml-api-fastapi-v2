...
class ProgressTracker:
    ...
    def _generate_progress_summary(self, history: List[ProgressPoint]) -> ProgressSummary:
        """Generar resumen de progreso."""
        if not history:
        ...
        if improvement_value > 0:
            improvement = f"+{improvement_value:.1f} puntos"
        elif improvement_value < 0:
            improvement = f"{improvement_value:.1f} puntos"
        else:
            improvement = "Sin cambios"
        ...
        # Calcular velocidad (mejora por día)
        if len(history) > 1:
            days_span = (datetime.fromisoformat(current.timestamp) - 
                        datetime.fromisoformat(history[0].timestamp)).days
            total_improvement = current_score - history[0].total_score
            velocity = total_improvement / max(days_span, 1)
        else:
            velocity = 0.0
        
        # Milestones y recomendaciones
        milestones_achieved = self._check_milestones(history)
        next_milestone = self._get_next_milestone(current_score)
    ...
    def _generate_recommendations(self, current: ProgressPoint, history: List[ProgressPoint]) -> List[str]:
    ...
    def _save_progress_summary(self, summary: ProgressSummary):
        ...
        print(f"📊 Progreso guardado: {summary.current_score:.1f} puntos ({summary.improvement})")
