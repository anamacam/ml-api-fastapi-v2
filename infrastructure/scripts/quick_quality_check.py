    ...
    def _check_file_basics(self, file_path: Path):
        ...
        try:
            ...
            try:
                ...
                self.errors.append(f"❌ {file_path}: Error de sintaxis en línea {e.lineno}")
            
            # Verificar longitud de líneas (básico)
            lines = content.split('\n')
            for i, line in enumerate(lines, 1):
                if len(line) > 120:  # Límite permisivo
                    self.warnings.append(f"⚠️ {file_path}:{i}: Línea muy larga ({len(line)} chars)")
            ...
            for node in tree.body:
                ...
                elif not isinstance(node, (ast.Expr, ast.Module)):
                    if isinstance(node.value if hasattr(node, 'value') else node, ast.Constant):
                        continue  # Docstring
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    if func_lines > 50:  # Umbral crítico
                        self.errors.append(
                            f"❌ {file_path}: Función '{node.name}' muy larga ({func_lines} líneas)"
                        )
                    elif func_lines > 30:  # Umbral de advertencia
                        self.warnings.append(
